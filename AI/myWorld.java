...
public class myWorld extends AbstractWorld 
{
	...
	Point friezaPos, gokuPos, dbPos;// positions of avatars
	...
	//a bool to say if we collected dragonball
	boolean dbCollected = false;
	...
	private transient InputStream friezaStream = null, gokuStream = null, caughtStream = null,bgStream = null, dragonballStream = null;		
    	private transient BufferedImage friezaImg, gokuImg, caughtImg,bgImg,dragonballImg; 
...
//==========================================================
	...
	private void addImage()	
	{
		...
		if (imagesDesired) 
		{
			...
			//=============================================
			if (friezaPos == gokuPos)
				img.createGraphics().drawImage ( caughtImg, (imgwidth * friezaPos.x), (imgheight * friezaPos.y), null );
			else
			{
	...
	public State getstate() throws RunError
	{
	  ...
	  String x = String.format ("%d,%d,%d,%d", friezaPos.x,friezaPos.y,gokuPos.x,gokuPos.y);
