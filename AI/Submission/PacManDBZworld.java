...
public class PacManDBZworld extends AbstractWorld 
{
	//this will be 16*16
	//yields 256 squares
	...
	Point friezaPos, gokuPos, dbPos, ginyuPos,cellPos;// positions of avatars
	int MAX_STEPS = 50;		// number of steps in a run
	...
	String IMG_CELL 	= SUPPORT_DIR + "/cell.png";
	...
	private transient InputStream friezaStream = null, gokuStream = null, cloudStream = null,bgStream = null, dragonballStream = null, deadStream = null, ginyuStream = null,superStream = null,lightStream = null, cellStream = null;		
    	private transient BufferedImage friezaImg, gokuImg, cloudImg,bgImg,dragonballImg,deadImg, ginyuImg,superImg,lightImg,cellImg; 
	...
	private void initPos()	
	{
		...
		{ 
			...
			cellPos = randomPosition();
		} 
		// repeat until different position
		while(friezaPos.equals(gokuPos) && ginyuPos.equals(gokuPos) &&
 			ginyuPos.equals(friezaPos) && cellPos.equals(gokuPos) && cellPos.equals(friezaPos) && cellPos.equals(ginyuPos));
	}
...
//==========================================================
			...
			// block is only executed once (only read from disk once)
	  		if(friezaStream == null) 					
	  		{
	   			...
	   			{	// use memory, not disk, for temporary images
					...
					cellStream = getClass().getResourceAsStream(IMG_CELL);
					...
					cellImg = javax.imageio.ImageIO.read(cellStream);
	...
	private void addImage()	
	{
		...
		if (imagesDesired) 
		{
			...
			//if the dragonball wasn't collected
			//then draw it on he map
			if(dbCollected == 0)
			{
			...
			//=============================================
			if (friezaPos.equals(gokuPos) || ginyuPos.equals(gokuPos) || cellPos.equals(gokuPos))
			{
				//if Goku hasn't collected the db
				//draw the dead img
				if(dbCollected == 0)				
				{
				...
				if(friezaPos.equals(gokuPos) && friezaDied == false && dbCollected > 0)
				{
					img.createGraphics().drawImage ( superImg, (imgwidth * gokuPos.x),(imgheight * gokuPos.y), null );
					if(cellDied == false)
					{
						img.createGraphics().drawImage ( cellImg, (imgwidth * cellPos.x),(imgheight * cellPos.y), null );
					}
					if(ginyuDied == false)
					{
				...
				else if(ginyuPos.equals(gokuPos) && ginyuDied == false && dbCollected > 0)
				{
					img.createGraphics().drawImage ( superImg, (imgwidth * gokuPos.x),(imgheight * gokuPos.y), null );
					if(cellDied == false)
					{
				...
				else if(cellPos.equals(gokuPos) && cellDied == false && dbCollected > 0)
				{
					img.createGraphics().drawImage ( superImg, (imgwidth * gokuPos.x),(imgheight * gokuPos.y), null );
					if(ginyuDied == false)
					{
			...
			//if the dB is collected
			//check who we need to draw and where
			if(dbCollected == 1)
			{
				if(friezaHealth == 1 && !friezaPos.equals(gokuPos))
				{
					img.createGraphics().drawImage ( friezaImg, (imgwidth * friezaPos.x), (imgheight * friezaPos.y), null );
				}
				if(ginyuHealth == 1 && !cellPos.equals(gokuPos))
				{
					img.createGraphics().drawImage ( ginyuImg, (imgwidth * ginyuPos.x),(imgheight * ginyuPos.y), null );
				}
				if(cellHealth == 1 && !cellPos.equals(gokuPos))
				{
					img.createGraphics().drawImage ( cellImg, (imgwidth * cellPos.x),(imgheight * cellPos.y), null );
				}
				if(gokuHealth > 0)
	...
	public void newrun() throws RunError
	{
		...
		scoreCols.add("NumBadGuys Killed");
	...
	public State getstate() throws RunError
	{
	  ...
	  String x = String.format ("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", friezaPos.x,friezaPos.y,gokuPos.x,gokuPos.y,dbPos.x,dbPos.y,ginyuPos.x,ginyuPos.y,dbCollected,friezaHealth,ginyuHealth,cellPos.x,cellPos.y,cellHealth);
	...
	{ 
		...
		if(friezaPos.equals(gokuPos) || ginyuPos.equals(gokuPos) || cellPos.equals(gokuPos))  
		{
			if((dbCollected == 0 && friezaPos.equals(gokuPos) && friezaHealth > 0) 
			    || (dbCollected == 0 && ginyuPos.equals(gokuPos) && ginyuHealth > 0)
			    ||	(dbCollected == 0 && cellPos.equals(gokuPos) && cellHealth > 0))
			{
				numCaught++;
			...
			{
				if(friezaPos.equals(gokuPos) && friezaHealth > 0)
				{
					friezaHealth = friezaHealth - 1;
					numBadGuys--;
					//if frieza dies move him to an invalid location
					addImage();
					friezaPos.move(0,0);
				}
				if(ginyuPos.equals(gokuPos) && ginyuHealth > 0)
				{
				...
				if(cellPos.equals(gokuPos) && cellHealth > 0)
				{
					cellHealth--;
					numBadGuys--;
					addImage();
	...
	{
		int numBadGuysKilled = 3 - numBadGuys;

		String s = String.format ("%d,%d,%d,%d", numCaught, gokuHealth, dbCount,numBadGuysKilled);
